...
LLVM_READONLY inline bool isHorizontalWhitespace(unsigned char c) {
  using namespace ch;
  return (CharTable[c] & (CHAR_HORZ_WS | CHAR_SPACE)) != 0;
}
...
LLVM_READONLY inline bool isVerticalWhitespace(unsigned char c) {
  using namespace ch;
  return (CharTable[c] & CHAR_VERT_WS) != 0;
}
...
LLVM_READONLY inline bool isWhitespace(unsigned char c) {
  using namespace ch;
  return (CharTable[c] & (CHAR_HORZ_WS | CHAR_VERT_WS | CHAR_SPACE)) != 0;
}
...
LLVM_READONLY inline bool isPreprocessingNumberBody(unsigned char c) {
  using namespace ch;
  return (CharTable[c] & (CHAR_UPPER | CHAR_LOWER | CHAR_DIGIT | CHAR_UNDER |
                          CHAR_PERIOD)) != 0;
}
...
LLVM_READONLY inline bool isRawStringDelimBody(unsigned char c) {
  using namespace ch;
  return (CharTable[c] & (CHAR_UPPER | CHAR_LOWER | CHAR_PERIOD | CHAR_DIGIT |
